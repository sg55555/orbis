...
# 全タイルの取得に許す上限。通常は約50秒で終わる（実測 0.44s/タイル）が、read timeout が
# 連鎖すると最悪 45×30秒＝22分まで伸びうる。collect は concurrency group を collect-slow と
...
# 交通密集地優先の 45 タイル（250NM 円）。候補 69 点を実測（2026-09-10）して確定：
...
#  - このセットで候補 69 点の 97.9%（4,615/4,715 機）を拾う。1 run あたり 45 req・約 50 秒。
TILES = (
    # 北米
    ...
    # 洋上・高緯度の穴埋め（北大西洋横断・太平洋・NZ）。1タイル16〜19機と薄いが、外すと
    # 収集範囲の lon が −125〜155 に収まり、地図の端（太平洋・北大西洋・NZ・アラスカ）が
    # 完全に無人に見える。ADS-B は地上受信機ベースなので外洋そのものは元々拾えない。
    ("atl-iceland",      64.1,  -21.9),
    ("pac-hawaii",       21.3, -157.9),
    ("nz-auckland",     -37.0,  174.8),
)
