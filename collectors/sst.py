...
def main():
    ...
    if aborted:
        print(f"[sst] aborted after consecutive failures ({failed} batches); keeping previous snapshot")
        return 1
    ...
    print(f"[sst] wrote {snap['count']}/{len(grid)} temps ({failed} batches missing) -> {snap_path}")
    return 0
