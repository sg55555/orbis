...
**前提:** 本番データには GitHub Secret `AISSTREAM_API_KEY` が必要（太田さんが直接設定）。キー無しでも全テストは緑になる設計（collector は skip、フロントはデータ無しで非表示）。
...
青い海・シアン航空との見分けやすさをスクショ比較。視認性とテーマ調和で太田さんと確定。
...
- [ ] **Step 4: 太田さんが GitHub Secret を設定**
