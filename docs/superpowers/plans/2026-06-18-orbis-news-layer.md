...
- 本番初期データは太田さんがローカルで `ANTHROPIC_API_KEY` を設定して `python -m collectors.news` を実行→`data/snapshots/news.json` を commit/push（vectors/サムネと同じローカル seed 手順）。または GitHub に `ANTHROPIC_API_KEY` Secret を設定し `gh workflow run collect.yml`。
