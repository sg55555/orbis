...
- date: 2026-06-19
- status: 設計確定（実装計画 待ち）
- related: live-translate v1（`/home/shugo/apps/live-translate`）, Obsidian `Projects/live-translate.md`, `Projects/orbis.md`
...
### 目的
太田さんが本番 Orbis（`https://orbis-beta.vercel.app/`）で外国語ニュースを視聴する際、
...
リポジトリ: `/home/shugo/apps/live-translate`
...
- **`?cap=` 系のノブは設けず**、実機で見ながら CSS を直接詰める（太田さんの実物比較スタイル）。最終位置/サイズ/不透明度は実装中の目視で確定。
