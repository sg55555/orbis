...
太田さん依頼：「Orbis 全体を見て追加機能を提案・実装」。本人優先＝① 日本語AI字幕の速度（品質維持）改善、② globe 国ドリルダウン（都市別詳細マッピング・是非含む）、③ 日本で市は出るが県が出ない違和感の解消。＋ Claude 推奨機能を並行検討。
...
- 県ラベルの実描画は GPU/フォント依存で headless 不可 → 太田さんの実機サニティに委ねる（mistakes: GPU依存は実機確認）
