...
- 初回＝本セッションで私が手元から同じ手順を実行（`git clone --depth 1` を scratchpad に → orphan → tree 一致 assert → `git push --force`）。**実行直前に AskUserQuestion で日本語の再確認**（不可逆・8,552 commits が消える）。main 統合後に `gh workflow run squash-data.yml -f confirm=squash` を 1 回打ち、workflow 本体も検証（1 commit を再 squash しても無害）。
...
- **`style-src-attr 'unsafe-inline'`**: コード変更ゼロだがスキャナー減点。19 箇所の機械置換で閉じられる（本人選択）。
