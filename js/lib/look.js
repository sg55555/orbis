...
// 主観的なダイヤル（大気の強さ・海陸の色・グラス透明度・深宇宙ベース色）をここに集約する。
// 採用が決まったら DEFAULT_LOOK を確定し、不要プリセットを整理する。
// sky.* は MapLibre v5 globe の setSky（大気ハロ）パラメータ。space は透明＝背面が透ける。
// 注: 星雲(面)は廃止したため nebula は深宇宙 vignette のベース色(base)のみ。
...
// ルックに応じた CSS 変数を :root に適用（深宇宙ベース色・グラス）。ブラウザのみ。
