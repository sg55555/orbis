...
test('Wave1（cyan border 単一用途）トークンが本文で参照されている', () => {
  // border 用途のみで使う cyan リテラル(.16/.18/.20)を視覚不変でトークン化した波。
  // boot(design-loading 領分)・mui の border は領分外で除外しているが、各トークンは
  // 少なくとも1箇所は本文 var() 参照されること（var-health が定義済を別途担保）。
  for (const t of ['--rim-cyan-16', '--rim-cyan-18', '--rim-cyan-20']) {
    assert.ok(css.includes(`var(${t})`), `${t} が未参照`);
  }
});
