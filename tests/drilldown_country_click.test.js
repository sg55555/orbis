...
test('handleMapClick: deck pick 直後＆近接座標なら openPlace を抑制', async () => {
  const map = fakeMap();
  let opened = 0;
...
test('openPlace: loadCountryGeo→resolvePlace→drill-open付与→resize→loadProfile→renderProfile→flyTo（国 target・reveal遅延）', async () => {
  const map = fakeMap();
  const order = [];
  ...
  // reveal（body+drill-open / state:loading）は loadCountryGeo→resolvePlace→target確定後に来る
  assert.deepEqual(order, ['load:JA', 'body+drill-open', 'state:loading', 'profile:JA', 'render', 'state:ready']);
...
test('openPlace: profilesManifest に存在しない国（resolvePlace が target:null）→ パネルを開かず onOceanMiss を呼ぶ', async () => {
  const map = fakeMap();
  let missCount = 0;
  let revealed = false;
  const rootEl = {
    removeAttribute(attr) { if (attr === 'hidden') revealed = true; },
    setAttribute() {},
    classList: { add() {}, remove() {} },
  };
  const bodyEl = {
    classList: {
      add(c) { if (c === 'drill-open') revealed = true; },
      remove() {},
    },
  };
  const api = initCountryClick({
    ...
    deps: baseDeps({
      rootEl,
      bodyEl,
      onOceanMiss: () => { missCount += 1; },
      // resolvePlace が target:null を返す（profilesManifest に当該 FIPS がない国）
      resolvePlace: () => ({ chain: [], target: null }),
      loadCountryGeo: async () => ({ admin1: { type: 'FeatureCollection', features: [] }, cities: [], degraded: true }),
      profilesManifest: { country: {}, admin1: {}, city: {} },
    }),
  });
  api.setBoundsPolys(POLYS);
  await api.openPlace(1, 1);   // POLYS 内 → JA と解決されるが profile 欠落
  assert.equal(missCount, 1, 'onOceanMiss が1回呼ばれた');
  assert.equal(revealed, false, 'パネルは開かれなかった（rootEl hidden 維持・body に drill-open なし）');
});
