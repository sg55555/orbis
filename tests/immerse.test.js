...
import {
  ...
  immerseSearch, immerseMobileUi, immerseSectionFit,
...
test('immerseClasses: 既定で seam-a・mbg-deep・mp-a・ui-a・font-on・sec-on・legend-on・search-on・feed-on・secfit-on。指定で上書き', () => {
  assert.deepEqual(immerseClasses(''), ['seam-a', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-b', 'secfit-on']);
  assert.deepEqual(immerseClasses('?seam=b'), ['seam-b', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-b', 'secfit-on']);
  assert.deepEqual(immerseClasses('?mbg=black'), ['seam-a', 'mp-a', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-b', 'secfit-on']);
  assert.deepEqual(immerseClasses('?seam=c&mbg=black&glass=off'), ['seam-c', 'glass-off', 'mp-a', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-b', 'secfit-on']);
  assert.deepEqual(immerseClasses('?glass=on'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-b', 'secfit-on']); // glass=on はクラス無し
  assert.deepEqual(immerseClasses('?mp=off'), ['seam-a', 'mbg-deep', 'mp-off', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-b', 'secfit-on']); // media before
  assert.deepEqual(immerseClasses('?ui=off&font=off'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-off', 'font-off', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-b', 'secfit-on']); // 本編 before
  assert.deepEqual(immerseClasses('?ui=b'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-b', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-b', 'secfit-on']); // 計器
  assert.deepEqual(immerseClasses('?sec=off'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-off', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-b', 'secfit-on']); // セクション before
});

test('immerseSectionFit: 既定 on（後付けセクション馴染ませ）。?secfit=off で上書き（無効も既定 on・大小無視）', () => {
  assert.equal(immerseSectionFit(''), 'on');
  assert.equal(immerseSectionFit('?secfit=on'), 'on');
  assert.equal(immerseSectionFit('?secfit=off'), 'off');
  assert.equal(immerseSectionFit('?secfit=OFF'), 'off'); // 大小無視
  assert.equal(immerseSectionFit('?secfit=x'), 'on');    // 不正は既定
  assert.equal(immerseSectionFit('?sec=off'), 'on');     // ?sec= とは独立（誤マッチしない）
});

test('immerseClasses: secfit- を末尾に常時付与（既定 secfit-on、?secfit=off で secfit-off）', () => {
  assert.ok(immerseClasses('').includes('secfit-on'));
  assert.ok(immerseClasses('?secfit=off').includes('secfit-off'));
  // ?sec=off は secfit に影響しない
  assert.ok(immerseClasses('?sec=off').includes('secfit-on'));
});
...
test('immerseSpace: 既定 off（不採用）。?space=1|2|3 で上書き（無効も既定off・大小無視）', () => {
  assert.equal(immerseSpace(''), 'off');
  assert.equal(immerseSpace('?space=1'), '1');
  assert.equal(immerseSpace('?space=2'), '2');
  assert.equal(immerseSpace('?space=3'), '3');
  assert.equal(immerseSpace('?space=off'), 'off');
  assert.equal(immerseSpace('?space=OFF'), 'off'); // 大小無視
  assert.equal(immerseSpace('?space=9'), 'off');   // 未定義段は既定
  assert.equal(immerseSpace('?space=x'), 'off');   // 不正は既定
});

test('immerseClasses: space- を常時付与（既定 space-off、?space=1|3 で上書き）', () => {
  assert.ok(immerseClasses('').includes('space-off'));
  assert.ok(immerseClasses('?space=1').includes('space-1'));
  assert.ok(immerseClasses('?space=3').includes('space-3'));
});
...
test('immerseMobileUi: 既定 b（採用）・?mui=a|off で上書き（無効も既定b・大小無視）', () => {
  assert.equal(immerseMobileUi(''), 'b');
  assert.equal(immerseMobileUi('?mui=a'), 'a');
  assert.equal(immerseMobileUi('?mui=b'), 'b');
  assert.equal(immerseMobileUi('?mui=off'), 'off');
  assert.equal(immerseMobileUi('?mui=OFF'), 'off'); // 大小無視
  assert.equal(immerseMobileUi('?mui=x'), 'b');     // 不正は既定(b)
});

test('immerseClasses: mui- を末尾に常時付与（既定 mui-b、?mui=off|a で上書き）', () => {
  assert.ok(immerseClasses('').includes('mui-b'));
  assert.ok(immerseClasses('?mui=off').includes('mui-off'));
  assert.ok(immerseClasses('?mui=a').includes('mui-a'));
});
