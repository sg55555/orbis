...
test('immerseClasses: 既定で seam-a・mbg-deep・mp-a・ui-a・font-on・sec-on・legend-on・search-on・feed-on。指定で上書き', () => {
  assert.deepEqual(immerseClasses(''), ['seam-a', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-a']);
  assert.deepEqual(immerseClasses('?seam=b'), ['seam-b', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-a']);
  assert.deepEqual(immerseClasses('?mbg=black'), ['seam-a', 'mp-a', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-a']);
  assert.deepEqual(immerseClasses('?seam=c&mbg=black&glass=off'), ['seam-c', 'glass-off', 'mp-a', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-a']);
  assert.deepEqual(immerseClasses('?glass=on'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-a']); // glass=on はクラス無し
  assert.deepEqual(immerseClasses('?mp=off'), ['seam-a', 'mbg-deep', 'mp-off', 'ui-a', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-a']); // media before
  assert.deepEqual(immerseClasses('?ui=off&font=off'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-off', 'font-off', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-a']); // 本編 before
  assert.deepEqual(immerseClasses('?ui=b'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-b', 'font-on', 'sec-on', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-a']); // 計器
  assert.deepEqual(immerseClasses('?sec=off'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-off', 'legend-on', 'search-on', 'feed-on', 'space-off', 'mui-a']); // セクション before
});
...
test('immerseMobileUi: 既定 a・?mui=b|off で上書き（無効も既定a・大小無視）', () => {
  ...
  assert.equal(immerseMobileUi('?mui=x'), 'a');     // 不正は既定
});

test('immerseClasses: mui- を末尾に常時付与（既定 mui-a、?mui=off|b で上書き）', () => {
