...
import {
  ...
  immerseMediaPolish, immerseUi, immerseFont, immerseSec,
...
test('immerseClasses: 既定で seam-a・mbg-deep・mp-a・ui-a・font-on・sec-on。指定で上書き', () => {
  assert.deepEqual(immerseClasses(''), ['seam-a', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-on']);
  assert.deepEqual(immerseClasses('?seam=b'), ['seam-b', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-on']);
  assert.deepEqual(immerseClasses('?mbg=black'), ['seam-a', 'mp-a', 'ui-a', 'font-on', 'sec-on']);
  assert.deepEqual(immerseClasses('?seam=c&mbg=black&glass=off'), ['seam-c', 'glass-off', 'mp-a', 'ui-a', 'font-on', 'sec-on']);
  assert.deepEqual(immerseClasses('?glass=on'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-on']); // glass=on はクラス無し
  assert.deepEqual(immerseClasses('?mp=off'), ['seam-a', 'mbg-deep', 'mp-off', 'ui-a', 'font-on', 'sec-on']); // media before
  assert.deepEqual(immerseClasses('?ui=off&font=off'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-off', 'font-off', 'sec-on']); // 本編 before
  assert.deepEqual(immerseClasses('?ui=b'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-b', 'font-on', 'sec-on']); // 計器
  assert.deepEqual(immerseClasses('?sec=off'), ['seam-a', 'mbg-deep', 'mp-a', 'ui-a', 'font-on', 'sec-off']); // セクション before
});
