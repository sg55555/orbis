...
test('buildBaseStyle: ラベルは name:ja→latin→en→name の順でフォールバック（現地スクリプト最終手段）', () => {
  const s = buildBaseStyle();
  const placeSymbols = s.layers.filter((l) => l['source-layer'] === 'place' && l.type === 'symbol');
  for (const l of placeSymbols) {
    const tf = JSON.stringify(l.layout['text-field']);
    assert.ok(tf.includes('name:ja'), 'name:ja を優先');
    assert.ok(tf.includes('name:latin'), 'ラテン文字フォールバック（アラビア語等の現地名直行を防ぐ）');
    assert.ok(tf.indexOf('name:ja') < tf.indexOf('name:latin'), 'name:ja が latin より優先');
  }
});
